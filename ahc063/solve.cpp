...
// Find and eat nearest reachable food (prefer correct color, accept any)
Route plan_eat_nearest_food(const State& s, int turn_base, const vector<int>& visit_count) {
    if (s.remaining == 0) return Route{};

    int free_t[MAX_CELLS];
    build_free_time(s, free_t);

    static int dist_buf[MAX_CELLS];
    static int prev_buf[MAX_CELLS];
    static signed char pdir_buf[MAX_CELLS];
    fill(dist_buf, dist_buf + MAX_CELLS, -1);
    ...
    int best_correct = -1, best_correct_d = INF_DIST;
    int best_any = -1, best_any_d = INF_DIST;
    ...
    while (!q.empty()) {
        ...
        for (int d = 0; d < 4; ++d) {
            if (!can_move(cur, d)) continue;
            int nxt = cur + DELTA[d];
            if (dist_buf[nxt] != -1) continue;
            int nd = cd + 1;
            if (nd < free_t[nxt]) continue;

            dist_buf[nxt] = nd;
            prev_buf[nxt] = cur;
            pdir_buf[nxt] = static_cast<signed char>(d);

            if (s.food[nxt] != 0) {
                if (s.len < M && s.food[nxt] == desired[s.len] && nd < best_correct_d) {
                    best_correct = nxt;
                    best_correct_d = nd;
                }
                if (nd < best_any_d) {
                ...
                continue; // don't expand through food
            }
            q.push(nxt);
        }
    }

    int target = (best_correct != -1) ? best_correct : best_any;
    if (target == -1) return Route{};

    // Reconstruct path
    vector<int> route;
    int cell = target;
    while (prev_buf[cell] != -1) {
        route.push_back(pdir_buf[cell]);
        cell = prev_buf[cell];
    }
    reverse(route.begin(), route.end());
    ...
    for (int dir : route) {
        StepInfo info;
        if (!step(end, dir, &info)) return Route{};
    }

    if (end.len <= s.len) return Route{};

    Route r;
    r.found = true;
    r.dirs = move(route);
    r.end = end;
    r.gain = (target == best_correct) ? 1 : 0;
    r.eval = future_eval(end, turn_base + static_cast<int>(r.dirs.size()), visit_count);
    if (r.gain > 0) r.eval += 30'000'000LL;
    return r;
}

// Plan a self-bite to free up space when completely stuck
Route plan_retreat_bite(const State& s, int turn_base, const vector<int>& visit_count) {
    if (s.len <= 6) return Route{};

    Route best;
    int head = s.head();

    for (int dir = 0; dir < 4; ++dir) {
        if (!can_move(head, dir)) continue;
        int nxt = head + DELTA[dir];
        if (s.len >= 2 && nxt == s.body[1]) continue;
        if (s.food[nxt] != 0) continue;
        ...
        for (int i = 2; i < s.len - 1; ++i) {
            if (s.body[i] == nxt) {
                ...
                r.end = end;
                r.gain = 0;
                r.eval = future_eval(end, turn_base + 1, visit_count)
                         + 500'000LL * info.cut_removed;
                if (r.eval > best.eval) best = move(r);
                break;
            }
        }
    }
...
// Multi-step bite planner - BFS to find a reachable body cell to bite
Route plan_approach_bite(const State& s, int turn_base, const vector<int>& visit_count) {
    if (s.len <= 8) return Route{};
    ...
    vector<BiteCandidate> candidates;
    ...
    while (!q.empty()) {
        ...
        for (int d = 0; d < 4; ++d) {
            ...
            // Check if nxt is a body cell that's still occupied at time nd
            if (s.food[nxt] == 0 && nd < free_t[nxt]) {
                // nxt is still a body cell - potential bite
                for (int i = 2; i < s.len - 1; ++i) {
                    if (s.body[i] == nxt) {
                        int est_cut = s.len - (i + 1);
                        if (est_cut > 2) {
                            candidates.push_back({cur, d, nd, est_cut});
                        }
                        break;
                    }
                }
    ...
    sort(candidates.begin(), candidates.end(), [](const BiteCandidate& a, const BiteCandidate& b) {
        if (a.cut_amount != b.cut_amount) return a.cut_amount > b.cut_amount;
        return a.dist < b.dist;
    });

    Route best;
    int check_limit = min<int>(8, (int)candidates.size());
    for (int ci = 0; ci < check_limit; ++ci) {
        auto& cand = candidates[ci];

        vector<int> route;
        int cell = cand.from_cell;
        while (prev_buf[cell] != -1) {
            route.push_back(pdir_buf[cell]);
            cell = prev_buf[cell];
        }
        reverse(route.begin(), route.end());
        route.push_back(cand.bite_dir);
        ...
        for (int dir : route) {
            StepInfo info;
            if (!step(end, dir, &info)) { valid = false; break; }
            if (info.cut_removed > 0) did_cut = true;
        }
        if (!valid || !did_cut) continue;
...
vector<char> solve(const State& initial, double deadline) {
    State s = initial;
    vector<char> ans;
    ans.reserve(100000);
    ...
    while (static_cast<int>(ans.size()) < 100000 && global_timer.elapsed() < deadline) {
        Route best_route;

        int exact_horizon = 12;
        double time_left = deadline - global_timer.elapsed();
        if (time_left < 0.45) exact_horizon = 10;
        if (time_left < 0.20) exact_horizon = 8;
        if (time_left < 0.10) exact_horizon = 5;

        // Phase 1: Try to find correct food
        Route exact = plan_exact(s, static_cast<int>(ans.size()), visit_count, rng, exact_horizon);
        if (exact.found) best_route = exact;

        if (s.len < M && time_left > 0.08) {
        ...
        // Phase 2: When stuck without eating, eat nearest food (any color)
        if (s.len < M && s.remaining > 0 && stagnation > N &&
            (!best_route.found || best_route.gain == 0)) {
            Route eat = plan_eat_nearest_food(s, static_cast<int>(ans.size()), visit_count);
            if (eat.found) {
                best_route = move(eat);
            }
        }

        // Phase 3: Endgame - aggressively eat any remaining food when time is low
        if (s.len < M && s.remaining > 0 && time_left < 0.20 &&
            (!best_route.found || best_route.gain == 0)) {
            Route eat = plan_eat_nearest_food(s, static_cast<int>(ans.size()), visit_count);
            if (eat.found) {
                best_route = move(eat);
        ...
        // Phase 4: If completely stuck, try biting tail
        if (!best_route.found || best_route.dirs.empty()) {
            Route bite = plan_retreat_bite(s, static_cast<int>(ans.size()), visit_count);
            if (bite.found) best_route = move(bite);
        }

        // Phase 5: Multi-step bite if still stuck
        if ((!best_route.found || best_route.dirs.empty()) && s.len > 8) {
            Route bite = plan_approach_bite(s, static_cast<int>(ans.size()), visit_count);
            if (bite.found) best_route = move(bite);
        }
        ...
        execute_route(
            ...
            stagnation, global_timer, deadline
        );
    }
...
int main() {
    ...
    constexpr double TOTAL_DEADLINE = 1.85;

    vector<char> best_ans = solve(initial, TOTAL_DEADLINE);

    for (char ch : best_ans) cout << ch << '\n';
    return 0;
}
